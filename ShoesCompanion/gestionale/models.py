...
# --- MODELLI DI BASE (Cliente, Colore) ---
...
# --- NUOVO MODELLO PER LE TAGLIE (SOSTITUISCE CoppiaMisura) ---
class Taglia(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('taglia_detail', kwargs={'pk': self.pk}) # Dovremo creare queste URL

# --- MODELLI PER LA STRUTTURA DELLA SCARPA ---
class Modello(models.Model):
    ...
    def duplicate(self, new_name=None):
        ...
        for componente_originale in self.componenti.all():
            ...
            # Duplica gli Articoli (le misure per taglia)
            for articolo_originale in componente_originale.articoli.all():
                Articolo.objects.create(
                    ...
                    descrizione_misura=articolo_originale.descrizione_misura
                )
        return new_modello
...
# --- NUOVO MODELLO DI MAPPING: Articolo (SOSTITUISCE MisuraComponente) ---
class Articolo(models.Model):
    ...
    def __str__(self):
        return f"{self.componente} per taglia {self.taglia}"

# --- MODELLI PER GLI ORDINI (AGGIORNATI) ---
class Ordine(models.Model):
    ...
    def get_materiali_necessari(self):
        ...
        for dettaglio in self.dettagli.all():
            taglia_ordinata = dettaglio.taglia
            quantita_per_taglia = dettaglio.quantita
            
            for componente_del_modello in self.modello.componenti.all():
                if Articolo.objects.filter(componente=componente_del_modello, taglia=taglia_ordinata).exists():
